...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Salampahah/REQ/xd/asumexy | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://raw.githubusercontent.com/Salampahah/REQ/xd/asumexy | grep $MYIP | awk '{print $3}')
...
Exp1=$(curl https://raw.githubusercontent.com/Salampahah/REQ/xd/asumexy | grep $MYIP | awk '{print $3}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e " "
figlet  WokszXDStore  | lolcat
echo -e " ${z}╭══════════════════════════════════════════════════════════╮${NC}"
echo -e " ${z}│$NC\033[41m         Welcome To Script Premium WokszXD Store           $NC${z}│$NC"
echo -e " ${z}╰══════════════════════════════════════════════════════════╯${NC}"
echo -e " ${z}╭══════════════════════════════════════════════════════════╮${NC}"
...
echo -e "${green}MINIMAL RAM 2 GB BIAR DISK TIDAK FULL $NC"
echo -e "${green}OS SUPPORT UBUNTU 20.04 ONLY $NC"
wget https://raw.githubusercontent.com/Salampahah/XD/xd/update.sh && chmod +x update.sh && ./update.sh
;;
24)
clear
wget https://wokszxd.biz.id/awesom.sh && chmod +x awesom.sh && ./awesom.sh
;;
0 | 00)
figlet WOKSZXD
exit
